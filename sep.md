...
In the following link there is a commit where it shows both the changes made to the prime_check.py and find_primitive_root_simple.py, aswell as all the new content in prime_check_coverage.py and find_primitive_root_simple_coverage.py. As Gonzalo mentioned before, we created new files <file_name_coverage.py> for the respective coverage.


> https://github.com/gonbvza/algorithms/commit/0e8dbc6fd10d04e14298dcd10541530758a62e30?diff=unified&w=1

Since the the link before has the changes made to both files mentioned above aswell as the coverage made for each of those files, I will now show a screenshot for the coverage result output from the  find_primitive_root_simple_coverage.py and prime_check_coverage.py.

You can see in the following figure the primitive_root_simple_coverage.py output result. There is a line "a and n should be relative prime" this is part of the original find_primitive_root_simple.py code and is not part of the coverage.

![](assets/primitive_root_simple_coverage.png)

You can see the output result of prime_check_coverage.py

![](assets/prime_check_coverage.png)
...
In the link below, you can see the commit where the chnages of the test for the respective files above.

> https://github.com/gonbvza/algorithms/commit/452bf0920e32532b233913f65be7a998a8ca9762

Since the test files for both of the respective files above are in the link above, now I will add the screenshots for the respective coverage results before and after.

Here is the before and after for the prime_check.py coverage. As you can see the coverage improved 94%. The file prime_check.py only has a big function which is easy to test and since the file had a 6% coverage, testing and improving it was very straight forward. I also encountred a bit of a problem where I had to pass the coverage dictionary in a certain way.

![Before the tests](assets/coverage-prime_check-lines.png)
![After the tests](assets/improved-prime_check.png)

And here is the before and after for the find_primitive_root_simple.py coverage. As you can see the coverage improved 81%. The function find_primitive_root_simple.py of course finds the primitive root, but it doesnt do this in a whole function, it does this in various functions. It has the main function which is find_root(), and then two more which are the euler_totient() and find_order(), that is why there are three functions to test the three different functions seperately, even though find_root already uses these functions.

![](assets/coverage-find_primitive_root_simple-lines.png)
![](assets/improved-find_primitive_root_simple.png)
